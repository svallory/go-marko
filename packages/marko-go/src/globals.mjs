...
import { readPackageVersion, versionSentinelName } from "./version.mjs";
...
/**
 * Render `globals.marko.go`. Always imports `runtime`: every generated file
 * references the codegen/runtime version sentinel (see version.mjs), and a
 * field's Go type may also mention `runtime.Body`.
 */
export function renderGlobalsFile(globals) {
  // The version sentinel (see version.mjs) always references `runtime`, so
  // this file always imports it now, regardless of whether a field's Go
  // type also happens to.
  const versionSentinel = versionSentinelName(readPackageVersion());
  const parts = [
    ...
    "",
    `// ${versionSentinel} asserts this file was generated by a marko-go whose`,
    "// minor version matches the runtime package below. A build error here",
    "// (\"undefined: runtime." +
      versionSentinel +
      '") means codegen and runtime have drifted -- update whichever is',
    "// behind so both are on the same major.minor version.",
    `var _ = runtime.${versionSentinel}`,
    "",
  ];
  parts.push(
