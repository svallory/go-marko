...
export function assembleFile(ctx, { moduleConstDecls, bodyGo }) {
  ...
  // Compile-time codegen/runtime lockstep check (see version.mjs). Zero
  // runtime cost -- the identifier is a const read whose value is discarded
  // -- but if the installed `runtime` package is a minor/major version
  // behind (or ahead of) what generated this file, `go build` fails with a
  // readable `undefined: runtime.GeneratedByMarkoGo_vX_Y` rather than a
  // confusing downstream type/API error.
